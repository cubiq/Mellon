...
class SD3MergeEmbeds(NodeBase):
    label = "SD3 Merge Embeddings"
    category = "embedding"
    params = {
        "embeds_1": { "label": "Embeddings 1", "type": "embedding", "display": "input" },
        "embeds_2": { "label": "Embeddings 2", "type": "embedding", "display": "input" },
        "factor": { "label": "Factor", "type": "float", "display": "slider", "default": 0.5, "min": 0.0, "max": 1.0, "step": 0.01 },
        "output": { "label": "Average", "type": "embedding", "display": "output" },
    }

    def execute(self, embeds_1, embeds_2, **kwargs):
        if not embeds_1 or not embeds_2:
            return { "output": None }
        
        # Calculate the average of the embeddings
        factor = kwargs.get("factor", 0.5)

        prompt_embeds = torch.lerp(embeds_1['prompt_embeds'], embeds_2['prompt_embeds'], factor)
        pooled_prompt_embeds = torch.lerp(embeds_1['pooled_prompt_embeds'], embeds_2['pooled_prompt_embeds'], factor)
        negative_prompt_embeds = torch.lerp(embeds_1['negative_prompt_embeds'], embeds_2['negative_prompt_embeds'], factor)
        negative_pooled_prompt_embeds = torch.lerp(embeds_1['negative_pooled_prompt_embeds'], embeds_2['negative_pooled_prompt_embeds'], factor)
    ...
    def slerp(v0, v1, t, DOT_THRESHOLD=0.9995):
        ...
        # Copy the vectors to avoid in-place operations
        v0 = v0.clone()
        v1 = v1.clone()

        # Normalize the vectors to get points on the hypersphere
        v0_norm = v0 / torch.norm(v0, dim=-1, keepdim=True)
        v1_norm = v1 / torch.norm(v1, dim=-1, keepdim=True)

        # Dot product of the vectors
        dot = torch.clamp(torch.sum(v0_norm * v1_norm, dim=-1, keepdim=True), -1.0, 1.0)
        
        # If the vectors are too close, fallback to linear interpolation
        ...
        v2 = v1 - v0 * dot
        v2_norm = torch.nn.functional.normalize(v2, dim=-1, eps=1e-8)
        
        # Return the interpolated vector
        return v0 * torch.cos(theta_t) + v2_norm * torch.sin(theta_t)
